...
/* Interrup Register Bits Definition */
				  ...
				  const int size)
{
	...
	while(inum < size && (reg_val & XUARTPS_SR_RXEMPTY) == 0) {
		rx_data[inum] = (u8_t)sys_read32(reg_base + XUARTPS_FIFO_OFFSET);
		inum++;
		reg_val = sys_read32(reg_base + XUARTPS_SR_OFFSET);
	}
...
static void uart_xlnx_ps_isr(void *arg)
{
	...
	struct uart_xlnx_ps_dev_data_t = DEV_DATA(dev);
...
static void uart_xlnx_ps_irq_callback_set(struct device *dev,
					  ...
					  void *cb_data)
{
	struct uart_xlnx_ps_dev_data_t * const dev_data = DEV_DATA(dev);
...
{ \
	IRQ_CONNECT(CONFIG_UART_XLNX_PS_##port##_IRQ, \
		    CONFIG_UART_XLNX_PS_##port##_IRQ_PRI, \
		    uart_xlnx_ps_isr, DEVICE_GET(uart_xlnx_ps_##port), \
		    0); \
	irq_enable(CONFIG_UART_XLNX_PS_##port##_IRQ); \
};
...
static struct uart_xlnx_ps_dev_config uart_xlnx_ps_dev_cfg_##port = { \
	.uconf = { \
		.regs = CONFIG_UART_XLNX_PS_##port##_BASEADDR, \
		.base = (u8_t *)CONFIG_UART_XLNX_PS_##port##_BASEADDR, \
		.sys_clk_freq = CONFIG_UART_XLNX_PS_##port##_CLK_FREQ, \
		UART_XLNX_PS_IRQ_CONF_FUNC_SET(port) \
	}, \
	.baud_rate = CONFIG_UART_XLNX_PS_##port##_BAUD_RATE, \
};

#ifdef CONFIG_UART_INTERRUPT_DRIVEN
#define UART_XLNX_PS_INIT(port) \
DEVICE_AND_API_INIT(uart_xlnx_ps, CONFIG_UART_XLNX_PS_##port##_NAME, \
		    &uart_xlnx_ps_init, \
...
#ifdef CONFIG_UART_XLNX_PS_0_BASEADDR
...
#ifdef CONFIG_UART_XLNX_PS_1_BASEADDR
