...
#if CONFIG_XLNX_PS_TTC_TIMER_INDEX == 0
#define TIMER_INPUT_CLKHZ   CONFIG_XLNX_PS_TTC_0_CLK_FREQ
#define TIMER_IRQ           CONFIG_XLNX_PS_TTC_0_IRQ
#define TIMER_BASEADDR      CONFIG_XLNX_PS_TTC_0_BASEADDR
#else
...
static int xttc_caluate_interval(u32_t *interval, u8_t *prescaler)
{
	u32_t tmpinterval = 0;
	u8_t tmpprescaler = 0;
	unsigned int tmpval;
...
int z_clock_driver_init(struct device *device)
{
	...
	/* Calculate prescaler */
	ret = xttc_caluate_interval(&interval, &prescaler);
	if (ret < 0) {
